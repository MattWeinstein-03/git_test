...
def is_freezing(celsius: float) -> bool:
    ...
    """
    # why: `celsius <= 0` already IS the boolean answer. Wrapping it in an
    # if/else to hand back True or False adds three lines and no meaning.
    return celsius <= 0
...
def sign_word(number: float) -> str:
    ...
    if number < 0:
        ...
        # why: `== 0` catches both 0 and 0.0, because they are equal values of
        # different types. Truthiness would work here too, but the explicit
        # comparison says what we mean.
        return "zero"
    else:
        return "positive"
...
def is_valid_percentage(value: float) -> bool:
    ...
    """
    # why: the chained form reads as a range, and `<=` on both ends is how you
    # spell "inclusive".
    return 0 <= value <= 100
...
def can_ride(height_cm: int, age: int, with_adult: bool) -> bool:
    ...
    """
    # why: the brackets are load-bearing. `and` binds tighter than `or`, so
    # without them the height rule would apply to the age branch only.
    return height_cm >= 120 and (age >= 12 or with_adult)
...
def starts_with_vowel(word: str) -> bool:
    ...
    """
    # why: the cheap protective test goes first. `and` short-circuits, so on an
    # empty string Python never evaluates word[0] and never raises IndexError.
    return len(word) > 0 and word[0].lower() in "aeiou"
...
def describe_stock(quantity: int | None) -> str:
    ...
    """
    # why: `is None` must come first. Once None is out of the way, every
    # remaining branch can safely compare `quantity` with a number — which a
    # None would refuse to do.
    if quantity is None:
        return "unknown"
    elif quantity < 0:
        return "invalid"
...
def pluralise(count: int, noun: str) -> str:
    ...
    """
    # why: `count != 1` rather than `count > 1`, so that 0 and -1 get the
    # plural too. The inner quotes are single because the f-string uses double.
...
def letter_grade(score: int) -> str:
    ...
    """
    # why: the guard clause fires first and leaves the chain below it dealing
    # with valid input only. `not 0 <= score <= 100` is the chained range test
    # from section 8, negated.
    if not 0 <= score <= 100:
        return "invalid"

    # why: highest band first. Each elif is only reached when every band above
    # it missed, so the lower bound alone is enough.
    if score >= 90:
        return "A"
    elif score >= 80:
        return "B"
...
def fizz_label(number: int) -> str:
    ...
    Examples:
        ...
        fizz_label(-9) -> "Fizz"
    """
    # why: the "both" test is the most specific case, so it goes at the top of
    # the chain. Written as two conditions joined by `and` rather than
    # `number % 15 == 0`, because that is what the rule actually says.
...
def ticket_price(age: int, is_member: bool, is_weekend: bool) -> float:
    ...
    """
    # why: guard clause first. Nonsense input leaves immediately, so nothing
    # below has to keep asking "but is the age sane?".
    if not 0 <= age <= 120:
        return 0.0
    ...
    # why: `price` is 0.0 for an under-5, and 0.0 is falsy — so `if price and
    # is_weekend` reads as "if there is a price at all, and it is the weekend".
    # An explicit `price > 0 and is_weekend` would be equally correct.
    if price and is_weekend:
        price += 2.0

    if is_member:
        price *= 0.9

    # why: round once, at the very end. 12 + 2 then * 0.9 gives
    # 12.600000000000001 in binary floating point (Day 2, section 3).
