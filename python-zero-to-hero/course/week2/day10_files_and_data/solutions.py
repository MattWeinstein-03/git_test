"""Day 10 solutions — Files and data.
...
def write_text_file(path: Path, text: str) -> int:
    ...
    """
    # why: exist_ok=True means "already there is fine", parents=True creates the
    # whole missing chain. Both keep this safe to call twice.
...
def read_lines(path: Path) -> list[str]:
    ...
    with open(path, encoding="utf-8") as file:
        # why: iterating the file object reads one line at a time, so memory use
        # does not grow with the file size.
        for line in file:
...
def append_line(path: Path, line: str) -> int:
    ...
    """
    path.parent.mkdir(parents=True, exist_ok=True)
    # why: mode "a" creates the file when absent and never truncates it.
    with open(path, "a", encoding="utf-8") as file:
        file.write(line + "\n")

    count = 0
    with open(path, encoding="utf-8") as file:
        for _existing_line in file:
            count += 1
    return count
...
def write_csv(path: Path, rows: list[dict[str, str]], fieldnames: list[str]) -> int:
    ...
    """
    path.parent.mkdir(parents=True, exist_ok=True)
    # why: newline="" stops Python translating line endings on top of what the
    # csv module already does — without it you get blank rows on Windows.
    with open(path, "w", encoding="utf-8", newline="") as file:
        writer = csv.DictWriter(file, fieldnames=fieldnames, restval="")
        writer.writeheader()  # nothing writes the header for you
...
def read_csv_rows(path: Path) -> list[dict[str, str]]:
    ...
    with open(path, encoding="utf-8", newline="") as file:
        ...
        for row in reader:
            # why: dict(row) makes a plain dict we own, independent of the reader.
...
def save_json(path: Path, data: object) -> None:
    ...
    path.parent.mkdir(parents=True, exist_ok=True)
    with open(path, "w", encoding="utf-8") as file:
        # why: dump (no "s") writes to a file object; dumps returns a string.
...
def load_json(path: Path, default: object = None) -> object:
    ...
    try:
        ...
        # why: named explicitly. A bare `except` here would also hide a typo in
        # this very function.
        return default
...
def clean_rows(
    ...
    for index, row in enumerate(rows):
        number = index + 1  # humans count rows from 1
        ...
        if missing:
            ...
            continue  # why: skip the row, keep importing the rest

        cleaned: dict[str, str] = {}
        for key, value in row.items():
            # why: build a NEW dict so the caller's data is never modified.
        ...
        for index, row in enumerate(reader):
            ...
            for field in numeric_fields:
                ...
                if raw is None or str(raw).strip() == "":
                    problem = f"row {number}: missing {field}"
                    break
                try:
                    numbers[field] = float(str(raw).strip())
                except ValueError:
                    # why: !r shows the quotes, so 'n/a' and '' are distinguishable.
                    problem = f"row {number}: {field} {raw!r} is not a number"
                    break
...
if __name__ == "__main__":
    ...
    try:
        print(write_text_file(scratch / "hello.txt", "hello"), "characters written")
        print(read_lines(scratch / "hello.txt"))
    finally:
