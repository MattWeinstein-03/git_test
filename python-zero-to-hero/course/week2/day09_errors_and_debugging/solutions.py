"""Day 09 solutions — Errors and debugging.
...
def safe_int(text: str, default: int = 0) -> int:
    ...
    try:
        ...
        # why: ValueError is exactly what int() raises for unparseable text.
        # Catching TypeError as well would hide callers passing None by mistake.
        return default
...
def get_nested(data: dict, keys: list[str], default: object = None) -> object:
    ...
    try:
        for key in keys:
            # why: one try around the whole walk is fine here because every
            # failure inside it means the same thing: "the chain is not there".
...
def parse_numbers(raw_values: list[str]) -> tuple[list[float], list[str]]:
    ...
    for value in raw_values:
        try:
            ...
            # why: report the culprit with !r so '' and ' ' are distinguishable.
            problems.append(f"{value!r} is not a number")
        else:
            # why: the append belongs in `else`, not in `try` — a failure here
            # would otherwise look like a parsing failure.
...
def validate_age(value: object) -> int:
    ...
    """
    # why: bool is a subclass of int, so it must be rejected explicitly.
    if isinstance(value, bool) or not isinstance(value, int):
        raise InvalidAgeError(
            f"age must be a whole number, got {type(value).__name__}"
        )
    if value < 0:
        raise InvalidAgeError(f"age must not be negative, got {value}")
    if value > 130:
        raise InvalidAgeError(f"age {value} is implausible")
    return value
...
def traced_divide(numerator: float, denominator: float) -> tuple[float | None, list[str]]:
    ...
    try:
        ...
        log.append("except TypeError")
    else:
        log.append("else")
    finally:
        # why: finally runs on every path, which is exactly what makes it the
        # right place for cleanup. Never `return` from here.
        log.append("finally")
    return result, log
...
def read_last_frame(traceback_text: str) -> tuple[str, int, str]:
    ...
    last_frame = ""
    exception_line = ""
    for raw_line in traceback_text.split("\n"):
        line = raw_line.strip()
        if not line:
            continue
        # why: the last non-blank line of a traceback is always the exception.
        exception_line = line
    ...
    # 'File "app.py", line 11, in f' -> ['File ', 'app.py', ', line 11, in f']
    pieces = last_frame.split('"', 2)
    filename = pieces[1]
    after_path = pieces[2]
    number_text = after_path.split("line ", 1)[1].split(",", 1)[0]
    return filename, int(number_text.strip()), exception_line
    ...
    * A number outside 0-100: raise ScoreError yourself with
    ...
    for name, value in raw.items():
        try:
            ...
            # why: `from error` keeps the ValueError as __cause__ so the
            # traceback shows both the friendly message and the real origin.
            raise ScoreError(f"bad score for {name!r}: {value!r}") from error
        if number < 0 or number > 100:
            raise ScoreError(f"score for {name!r} out of range: {number}")
        scores[name] = number
    return scores
...
# ---------------------------------------------------------------------------
def _split_setting(line: str) -> tuple[str, str]:
    """Return (key, value) for one config line, or raise ConfigError.

    The raised message is a fragment — the caller adds the line number and the
    original text — which keeps every parsing RULE in this one function.
    """
    if "=" not in line:
        raise ConfigError("missing '='")
    key_text, value_text = line.split("=", 1)  # split on the FIRST = only
...
def parse_config(lines: list[str]) -> tuple[dict[str, str], list[str]]:
    ...
    * Lowercase the key, so "DEBUG" and "debug" are the same setting.
    ...
    for index, raw_line in enumerate(lines):
        ...
        try:
            key, value = _split_setting(line)
        except ConfigError as error:
            # why: the helper knows the rule, the loop knows the position.
            problems.append(f"line {number}: {error} in {line!r}")
            continue
